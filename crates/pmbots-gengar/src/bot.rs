...
impl GengarBot {
    ...
    pub async fn resolve_pending_buy(&self) {
        ...
        let real_bal = match exec
            .clob
            .get_balance_allowance(&exec.creds, exec.auth_address, exec.sig_type)
            .await
        {
            Ok(b) => b,
            Err(e) => {
            ...
            Failed(reason) => {
                ...
                if reason.contains("UNVERIFIED_BUY") {
                    ...
                    let balance_before = exec_ref
                        .clob
                        .get_balance_allowance(&exec_ref.creds, exec_ref.auth_address, exec_ref.sig_type)
                        .await
                        .unwrap_or(0.0);
    ...
    pub async fn resolve_pending_phantom(&self) {
        ...
        let exec = self.executor.as_ref().unwrap();
        let real_bal = match exec.clob.get_balance_allowance(&exec.creds, exec.auth_address, exec.sig_type).await {
            Ok(b) => b,
            Err(e) => {
    ...
    pub async fn resolve_window(&self, prev_window_ts: i64, prev_opening_price: f64) {
        ...
        for pos in prev_positions {
            ...
            let exec = self.executor.as_ref().unwrap();
            let balance_before = exec.clob.get_balance_allowance(&exec.creds, exec.auth_address, exec.sig_type).await.unwrap_or(0.0);
